...
// Card Dimensions (440-460px W x 270-290px H) & Opposing Rotations
const CARD_VARIANTS = [
  { sizeClass: 'w-[440px] sm:w-[460px] h-[270px] sm:h-[290px]', rotClass: 'rotate-[-2deg]', photoRotClass: 'rotate-[3deg]', tapeRot: -10 },
  { sizeClass: 'w-[435px] sm:w-[455px] h-[265px] sm:h-[285px]', rotClass: 'rotate-[1deg]', photoRotClass: 'rotate-[-3deg]', tapeRot: 8 },
  { sizeClass: 'w-[445px] sm:w-[460px] h-[272px] sm:h-[292px]', rotClass: 'rotate-[2deg]', photoRotClass: 'rotate-[-2deg]', tapeRot: -12 },
  { sizeClass: 'w-[438px] sm:w-[458px] h-[268px] sm:h-[288px]', rotClass: 'rotate-[-1deg]', photoRotClass: 'rotate-[4deg]', tapeRot: 14 },
  { sizeClass: 'w-[442px] sm:w-[462px] h-[270px] sm:h-[290px]', rotClass: 'rotate-[3deg]', photoRotClass: 'rotate-[-3deg]', tapeRot: -8 }
];
...
export const ReviewCard: React.FC<ReviewCardProps> = React.memo(({ item, index }) => {
  ...
  return (
    <article
      className={`${variant.sizeClass} shrink-0 ${cardConfig.bg} ${cardConfig.border} border-[4px] shadow-[8px_8px_0_#2F221D] rounded-[26px] p-6 sm:p-7 relative transform ${variant.rotClass} hover:rotate-0 hover:-translate-y-2.5 hover:scale-[1.03] hover:shadow-[12px_12px_0_#2F221D] transition-all duration-300 ease-out cursor-pointer z-10 hover:z-30 select-none flex items-center justify-between group overflow-visible`}
    >
      ...
      <MaskingTape 
        ...
        className="absolute -top-4 left-10 z-20 opacity-90 pointer-events-none" 
      />

      {/* TOP RIGHT: Micro SVG Scrapbook Decoration */}
      <div className="absolute top-4 right-6 opacity-75 pointer-events-none">
        {index % 4 === 0 && (
          <svg className="w-5 h-5 text-[#FFE066]" fill="currentColor" viewBox="0 0 24 24">
        ...
        {index % 4 === 2 && (
          <svg className="w-5 h-5 text-white/80 -rotate-12" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 32">
            <path strokeLinecap="round" strokeLinejoin="round" d="M7 8v14a5 5 0 0010 0V6a3 3 0 00-6 0v14a1 1 0 002 0V8" />
          </svg>
        )}
        {index % 4 === 3 && (
          <span className="font-fasthand text-xs font-bold text-white/90 rotate-[-6deg] inline-block border border-dashed border-white/80 px-1.5 py-0.5 rounded bg-black/20">
            Fresh!
          </span>
        )}
      </div>
      ...
      <div className="w-[58%] sm:w-[60%] shrink-0 pr-2 flex flex-col justify-between h-full py-1">
        <div>
          {/* STEP 1: TOP LEFT - ★★★★★ Rating Stars */}
          <div className="flex items-center gap-1 mb-2">
            {[...Array(item.rating || 5)].map((_, i) => (
              <svg key={i} className="w-4.5 h-4.5 text-[#FFE066] fill-current filter drop-shadow-[0.5px_0.5px_0_#2F221D]" viewBox="0 0 24 24">
          ...
          <cite className={`font-fraunces text-xl sm:text-2xl font-bold ${cardConfig.text} not-italic tracking-tight block mb-2 filter drop-shadow-[0.5px_0.5px_0_rgba(0,0,0,0.15)]`}>
            {item.name}
          </cite>

          {/* STEP 3: Review Text in Mali (Adaptive Text Color) */}
          <blockquote className={`font-mali text-xs sm:text-sm ${cardConfig.text} font-semibold leading-[1.65] line-clamp-4`}>
        ...
        <div className="flex items-center gap-1 pt-2">
          <span className={`font-fasthand text-xs sm:text-sm font-bold ${cardConfig.subText}`}>
            {labelText}
          </span>
        </div>
      </div>
      ...
      <div className="w-[42%] sm:w-[40%] h-full relative flex items-center justify-end pointer-events-none pr-1">
        ...
        <div className="relative -bottom-3.5 -right-3.5 z-10">
          
          {/* Unified Warm Ivory (#F5EFE6) Abstract Paper Shape (120-130% W, 110-120% H centered behind portrait) */}
          <div className="absolute -inset-5 sm:-inset-7 flex items-center justify-center -z-10 transform group-hover:scale-105 transition-transform duration-300 opacity-98 pointer-events-none">
          ...
          <div className={`w-32 sm:w-36 h-36 sm:h-40 rounded-2xl overflow-hidden border-[3.5px] border-white bg-[#FAF4E8] shadow-[6px_6px_0_rgba(0,0,0,0.25)] transform ${variant.photoRotClass} group-hover:rotate-0 transition-transform duration-300 relative z-10`}>
            <img 
              src={item.portrait} 
              alt={item.name} 
          ...
          <MaskingTape color="kraft" width={38} height={14} rotate={-14} className="absolute -top-3 left-2 z-20 opacity-90 pointer-events-none" />
